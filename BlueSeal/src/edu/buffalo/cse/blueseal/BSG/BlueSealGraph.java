...
public class BlueSealGraph {
	...
	public int size(){
		return argNodes.size()+retNodes.size() 
				+srcNodes.size()+sinkNodes.size()
				+cvNodes.size() + trans.size();
	}
	...
	public BlueSealGraph(BlueSealGraph g){
		...
		if(!this.trans.isEmpty()) this.trans.removeAll(cvNodes);
		...
		this.trans.addAll(g.getTansNodes());
		
	}
	
	public int hashCode(){
		return edges.hashCode()+argNodes.hashCode()+retNodes.hashCode()+
				srcNodes.hashCode()+sinkNodes.hashCode()+cvNodes.hashCode()+
				trans.hashCode();
	}
	...
	public List<String> generateFlowStrings(){
		...
		for(Edge e:this.edges){
			...
			String eString = e.generateFlowString();
			list.add(eString);
		}
	...
	public void printFlow(){
		...
		for(Edge e:this.edges){
			...
			System.out.println("Flow #"+((Integer)i).toString()+":");
			e.printFlow();
			i++;
		}
	}
...
/*		
	...
	public Set<Edge> getEdgesOutOf(Node orig) {
	...
	public Set<Edge> getEdgesInto(Node orig) {
		// find all the edges out of the curren node
		Set<Edge> out = new HashSet<Edge>();
		for(Iterator it=edges.iterator();it.hasNext();){
			Edge edge = (Edge)it.next();
	...
	public void merge(BlueSealGraph g) {
		this.edges.addAll(g.getEdges());
		this.argNodes.addAll(g.getArgNodes());
		this.retNodes.addAll(g.getRetNodes());
		this.srcNodes.addAll(g.getSrcs());
	...
	public Set<Node> getSuccessorsOf(Node node) {
