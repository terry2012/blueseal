...
public class BSForwardFlowAnalysis extends ForwardFlowAnalysis {
	...
	public static Map<FieldRef, ArraySparseSet> CVFlowSet = 
			new HashMap<FieldRef, ArraySparseSet>();
	public Map<Unit, BlueSealGraph> unitToBSG = 
			new HashMap<Unit, BlueSealGraph>();

	public BSForwardFlowAnalysis(DirectedGraph graph,
			AbstractInterproceduralAnalysis inter_, SootMethod method){
		super(graph);
		this.inter = inter_;
		this.method_ = method;
	 ...
	 * compute the information of each stmt and build up bluesealgraph
	 */
	@Override
...
//			System.out.println("[BlueSeal-flowAnalysis]:source detected@" +
//					stmt+ "->"+ this.method_.getName()+"::"+this.method_.getDeclaringClass().getName());
...
//			System.out.println("[BlueSeal-flowAnalysis]:sink detected@" +
//					stmt+ "->"+ this.method_.getName()+"::"+this.method_.getDeclaringClass().getName());
		 ...
		 */
			...
			if(value instanceof ParameterRef){
				...
				ArgNode arg = new ArgNode(stmt.toString(), stmt, method_, para_index);
				bsg.addArgNode(arg);
		 ...
		 */
			...
			RetNode ret = new RetNode(stmt.toString(), stmt, method_);
			bsg.addRetNode(ret);
